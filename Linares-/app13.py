...
def fiesta_de_fin_de_ciclo():
    lugar=libreria.pedir_universidad("universidad:")
    precio_de_la_entrada=libreria.pedir_numero("precio de entradas en soles:", 50, 60)
    ciclo=libreria.pedir_numero("ciclo:", 1, 10)
    contenido=lugar + "-" + str(precio_de_la_entrada) + "-" + str(ciclo) + "\n"
    libreria.guardar_datos("info.txt", contenido, "a")
    print("Datos guardados")
...
def opcionB():
    ...
    max=3
    while (opc!= max):
        print("######### Fiesta de fin de ciclo ########")
        print("1. fista de fin de ciclo")
        print("2. Var datos")
        print("3. Salir")
        print("#######################")
        opc =libreria.pedir_numero("Ingrese subopcion: ", 1, 3)
        if( opc == 1):
            fiesta_de_fin_de_ciclo()
        if( opc == 2):
            verDatos()
